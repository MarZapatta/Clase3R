...
# es notorio que la variable $date debe ser una que represente a la linea del tiempo
class(storms$date)
storms$date <- as.Date(storms$date)

#### Funcion select ####
#permite sleecion columnas 2da dimension de un data frame
...
#para exclui una columna utilzamos el operado unario
select(storms,-storm)
...
#dplyr nos permitera seleccionar columnas que esten con un patron ()
...
#el sgte ejm filtra aquellas fils con wind>=50 y pressure <1010
...
#### mutate ####
#con la fuincion mutate podemos computar calcular transformaciones de variables
# de un data frame. a menudo tendremos la necesidad de crear nuevas variables
#que se calculan a partir de variables exiwstente , murate() proporciona
